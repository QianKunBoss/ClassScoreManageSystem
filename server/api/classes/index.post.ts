...
export default defineEventHandler(async (event) => {
  ...
  if (!gradeId || isNaN(Number(gradeId))) {
    throw createError({ statusCode: 400, message: '年级ID无效' })
  }

  if (!name || typeof name !== 'string' || !name.trim()) {
    throw createError({ statusCode: 400, message: '班级名称不能为空' })
  }
  ...
  if (!grade) {
    throw createError({ statusCode: 404, message: '年级不存在' })
  }
  ...
  if (admin.role === 'grade_admin' && admin.gradeId !== gradeIdNum) {
    throw createError({ statusCode: 403, message: '只能为自己年级创建班级' })
  }
  ...
  if (existing) {
    throw createError({ statusCode: 400, message: '该年级下已存在同名班级' })
  }
